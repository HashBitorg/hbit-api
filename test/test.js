describe('HBIT', function(){

    it('exists', function(){
        expect(HBIT).to.be.ok;
        expect(HBIT.API).to.be.ok;
        expect(HBIT.ErrorCodes).to.be.ok;
    });

    describe('API', function(){
        beforeEach(function(){
            this.instance = new HBIT.API('http://127.0.0.1:9999');
        });

        it('has endpoint defined', function(){
            expect(this.instance.config).to.be.an('object').that.deep.equal({
                url: 'http://127.0.0.1:9999/hbit',
                json: true
            });
        });

        it('responds to all ' + (Object.keys(HBIT.API.prototype).length) + ' APIs defined with non error', function(){
            var
                i,
                promises = [],
                self = this,
                names = Object.keys(HBIT.API.prototype),
                skip = ['_call','_clean'].concat(Object.keys(this.instance));


            sinon.stub(this.instance.request, 'post', function(args, callback){
                expect(args.url).to.equal('http://127.0.0.1:9999/hbit');
        ...
        it('responds with errorCode and errorDescription', function(){
            ...
            .then(function(){
                ...
                return self.instance.getBlockchainStatus({error: {code: 1, desc: HBIT.ErrorCodes[1]}});
            })
            .then(function(){
                expect(function(){}).to.throw(/Should not succeed/);
            }, function(err){
                expect(err).to.deep.equal({errorCode: 1, errorDescription: HBIT.ErrorCodes[1]});
        ...
        it('doesnt double append /hbit', function(){
            var instance = new HBIT.API('http://127.0.0.1:9999/hbit');

            expect(instance.config.url).to.match(/\/hbit$/);
        });
